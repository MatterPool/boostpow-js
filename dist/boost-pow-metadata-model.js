...
class BoostPowMetadataModel {
  ...
  getTagUtf8() {
    return this.trimBufferString(
      Buffer.from(this.tag).reverse().toString("hex"),
      true
    );
  }
  getTagString() {
  ...
  getUserNonceUtf8() {
    return this.trimBufferString(
      Buffer.from(this.userNonce).reverse().toString("hex"),
      true
    );
  }
  getUserNonceNumber() {
  ...
  getAdditionalDataUtf8() {
    return this.trimBufferString(
      Buffer.from(this.additionalData).reverse().toString("hex"),
      true
    );
  }
  ...
  toObject() {
    return {
      ...
      minerPubKeyHash: (this.minerPubKeyHash.toString("hex").match(/../g) || [])
        .reverse()
        .join(""),
