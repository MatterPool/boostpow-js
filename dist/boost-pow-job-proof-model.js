...
class BoostPowJobProofModel {
  constructor(signature, minerPubKey, time, extraNonce1, extraNonce2, nonce,
              minerPubKeyHash,
  ...
  static fromObject(params) {
    ...
    if (params.minerPubKeyHash) {
      ...
      minerPubKeyHash =
          bsv.crypto.hash.sha256ripemd160(minerPubKey).toString('hex');
    }
    return new BoostPowJobProofModel(
        Buffer.from(params.signature, 'hex'), minerPubKey,
  ...
  getTimeNumber() { return this.time.readUInt32LE(); }
  getTimeBuffer() { return this.time; }
  setTime(time) {
  ...
  getNonceNumber() { return this.nonce.readUInt32LE(); }
  ...
  setExtraNonce1(nonce) {
    this.extraNonce1 =
        boost_utils_1.BoostUtils.createBufferAndPad(nonce, 4, false);
  }
  setExtraNonce2(nonce) {
    this.extraNonce2 =
        boost_utils_1.BoostUtils.createBufferAndPad(nonce, 8, false);
  }
  // Should add bsv.Address version and string version too
  getMinerPubKeyHash() { return this.minerPubKeyHash; }
  getMinerPubKeyHashHex() { return this.minerPubKeyHash.toString('hex'); }
  setSignature(signature) { this.signature = Buffer.from(signature, 'hex'); }
  setSignatureBuffer(signature) { this.signature = signature; }
  getSignature() { return this.signature; }
  getSignatureHex() { return this.signature.toString('hex'); }
  getMinerPubKey() { return this.minerPubKey; }
  getMinerPubKeyHex() { return this.minerPubKey.toString('hex'); }
  /**
   *
    ...
    for (const input of tx.inputs) {
      try {
        return BoostPowJobProofModel.fromScript(
            input.script, tx.hash, inp, input.prevTxId.toString('hex'),
            input.outputIndex); // spentTx, spentVout);
      } catch (ex) {
  ...
  static fromScript(script, txid, vin, spentTxid, spentVout) {
    return BoostPowJobProofModel.fromHex(script, txid, vin, spentTxid,
                                         spentVout);
  }
  static fromHex(asm, txid, vin, spentTxid, spentVout) {
    ...
    if (7 === script.chunks.length &&
      ...
      return new BoostPowJobProofModel(
          signature, minerPubKey, time, extraNonce1, extraNonce2, nonce,
          minerPubKeyHash, txid, vin, spentTxid, spentVout);
    }
  ...
  getSpentTxid() { return this.spentTxid; }
  // Optional attached information if available
  getSpentVout() { return this.spentVout; }
  toASM() {
