...
class BoostPowJobModel {
  ...
  getContentString(trimTrailingNulls = true) {
    return boost_utils_1.BoostUtils.trimBufferString(this.content,
                                                     trimTrailingNulls);
  }
  getContentHex() {
  ...
  getCategoryString(trimTrailingNulls = true) {
    return boost_utils_1.BoostUtils.trimBufferString(this.category,
                                                     trimTrailingNulls);
  }
  getTagString(trimTrailingNulls = true) {
    return boost_utils_1.BoostUtils.trimBufferString(this.tag,
                                                     trimTrailingNulls);
  }
  getTagHex() { return this.tag.toString("hex"); }
  getTagBuffer() { return this.tag; }
  getAdditionalDataString(trimTrailingNulls = true) {
    return boost_utils_1.BoostUtils.trimBufferString(this.additionalData,
                                                     trimTrailingNulls);
  }
  ...
  static fromOpCode(chunk) {
    if (chunk.opcodenum >= bsv.Opcode.OP_1 &&
  ...
  static createRedeemTransaction(boostPowJob, boostPowJobProof, privateKeyStr,
    ...
    if (!boostPowJob.getTxid() || boostPowJob.getVout() === undefined ||
        boostPowJob.getVout() === null || !boostPowJob.getValue()) {
      throw new Error(
