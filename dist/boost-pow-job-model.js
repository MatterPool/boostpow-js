...
class BoostPowJobModel {
  ...
  static fromOpCode(chunk) {
    if (
      chunk.opcodenum >= bsv.Opcode.OP_1 &&
      chunk.opcodenum <= bsv.Opcode.OP_16
    ) {
  ...
  static readScript(script, txid, vout, value) {
    ...
    if (
      ...
      ((script.chunks[5].buf && script.chunks[5].len <= 20) ||
        (script.chunks[5].opcodenum >= bsv.Opcode.OP_1 &&
          script.chunks[5].opcodenum <= bsv.Opcode.OP_16)) &&
      // User Nonce
      script.chunks[6].buf &&
      script.chunks[6].len === 4 &&
      // Additional Data
      (script.chunks[7].buf ||
        (script.chunks[7].opcodenum >= bsv.Opcode.OP_1 &&
          script.chunks[7].opcodenum <= bsv.Opcode.OP_16))
    ) {
      if (
        BoostPowJobModel.remainingOperationsMatchExactly(
          script.chunks,
          8,
      ...
      let targetHex = (
        this.fromOpCode(script.chunks[4]).toString("hex").match(/../g) || []
      )
        .reverse()
        .join("");
      let targetInt = parseInt(targetHex, 16);
  ...
  static fromASM(asm, txid, vout, value) {
    return BoostPowJobModel.readScript(
      new bsv.Script.fromASM(asm),
      txid,
      vout,
      value
    );
  ...
  static fromTransaction(tx, vout = 0) {
    ...
    if (
      ...
      return BoostPowJobModel.fromScript(
        tx.outputs[vout].script,
        tx.hash,
        vout,
  ...
  static createRedeemTransaction(
    ...
    const boostPowString = BoostPowJobModel.tryValidateJobProof(
      boostPowJob,
      boostPowJobProof
    );
    if (!boostPowString) {
  ...
  static tryValidateJobProof(boostPowJob, boostPowJobProof, debug = false) {
    ...
    if (debug) {
      ...
      console.log(
        "content",
        boostPowJob.getContentBuffer().toString("hex"),
      ...
      console.log(
        "time",
        boostPowJobProof.getTime().toString("hex"),
      ...
      console.log(
        "nonce",
        boostPowJobProof.getNonce().toString("hex"),
    ...
    if (blockHeader.validProofOfWork()) {
      ...
      return {
        boostPowString: new boost_pow_string_model_1.BoostPowStringModel(
          blockHeader
        ),
