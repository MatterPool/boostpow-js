...
describe("boost #BoostPowString validateProofOfWork ", () => {
  ...
  it("getTargetDifficulty success", async () => {
    ...
    expect(targetDiff.toHex())
        .to.eql("6a93b30000000000000000000000000000000000000000000000");
  });
...
describe("BoostPowString", () => {
  it("should get correctly accessors", async () => {
    ...
    expect(boostPowString.contentHex())
        .to.eql(
            "00000000000000000000000000000000000000000048656c6c6f20776f726c64");
    expect(boostPowString.contentBuffer().toString("hex"))
        .to.eql(
            "646c726f77206f6c6c6548000000000000000000000000000000000000000000");
    expect(boostPowString.contentString(false))
        .to.eql(
            "dlrow olleH\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000\u0000");
    ...
    expect(boostPowString.metadataHash())
        .to.eql(
            "acd8278e84b037c47565df65a981d72fb09be5262e8783d4cf4e42633615962a");
...
describe("boost integration test ", () => {
  ...
  const contentBuffer =
      index.BoostUtilsHelper.stringToBuffer(contentString, 32);
  ...
  it("should create the correct locking script",
     async () => { expect(lockingScript).to.eql(expectedLockingScript); });

  it("should create the correct unlocking script",
     async () => { expect(unlockingScript).to.eql(expectedUnlockingScript); });
  ...
  it("should get miner pubkey hash from solution", async () => {
    ...
    expect(solution.getMinerPubKeyHashHex().toUpperCase())
        .to.eql(minerPubKeyHashHex);
  });
  ...
  it("should generate valid string", async () => {
    ...
    expect(proof.boostPowString.toString().toUpperCase())
        .to.eql(proofStringHex);
    ...
    expect(proof.boostPowString.metadataHash().toUpperCase())
        .to.eql(metadataHashHex);
  });

  it("should write as objects and back", async () => {
    expect(index.BoostPowJob.fromObject(job.toObject())).to.eql(job);
    expect(index.BoostPowMetadata.fromObject(metadata.toObject()))
        .to.eql(metadata);
  });
});
